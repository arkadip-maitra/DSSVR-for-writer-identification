...
pretrain_save_path = "results_rthw/RTHW_NewLossFn_model.pth"
finetune_save_path = "results_rthw/RTHW_NewLossFn_model_finetuned.pth"
results_df_save_path = "results_rthw/RTHW_NewLossFn_model_finetune_results_rthw.csv"
pretrain_image_save_path = "results_rthw/RTHW_NewLossFn_pretrain_loss.png"
finetune_image_save_path = "results_rthw/RTHW_NewLossFn_finetune_loss.png"
...
class newlossfn(nn.Module):
    ...
    def forward(self, z1, z2):
        N = 2 * self.batch_size
        z1 = torch.nn.functional.normalize(z1, dim=0, p=2)
        z2 = torch.nn.functional.normalize(z2, dim=0, p=2)
        z1mod = (z1 - z1.mean(dim = 0)) / torch.std(z1, dim=0)
        z2mod = (z2 - z2.mean(dim = 0)) / torch.std(z2, dim=0)
